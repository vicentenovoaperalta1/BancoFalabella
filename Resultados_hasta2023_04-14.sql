...
WITH AC_FINTOC AS (SELECT RUT AS IdCliente, 1 AS Aumento_Fintoc,
     ...
     AGRUPACION AS (SELECT AA.FechaFoto AS FechaAumento,
                           ...
                           CASE WHEN DATE_DIFF(CC.FechaFoto, AA.FechaFoto, MONTH) = -1  AND MoraAc >= 30 THEN FSaldeud
                                WHEN DATE_DIFF(CC.FechaFoto, AA.FechaFoto, MONTH) =  0  AND MoraAc >= 30 THEN FSaldeud
                                WHEN DATE_DIFF(CC.FechaFoto, AA.FechaFoto, MONTH) =  1  AND MoraAc >= 30 THEN FSaldeud
                                WHEN DATE_DIFF(CC.FechaFoto, AA.FechaFoto, MONTH) =  2  AND MoraAc >= 30 THEN FSaldeud
                                WHEN DATE_DIFF(CC.FechaFoto, AA.FechaFoto, MONTH) =  3  AND MoraAc >= 30 THEN FSaldeud
                                WHEN DATE_DIFF(CC.FechaFoto, AA.FechaFoto, MONTH) =  4  AND MoraAc >= 30 THEN FSaldeud
                                WHEN DATE_DIFF(CC.FechaFoto, AA.FechaFoto, MONTH) =  5  AND MoraAc >= 30 THEN FSaldeud
                                WHEN DATE_DIFF(CC.FechaFoto, AA.FechaFoto, MONTH) =  6  AND MoraAc >= 30 THEN FSaldeud
                                WHEN DATE_DIFF(CC.FechaFoto, AA.FechaFoto, MONTH) =  7  AND MoraAc >= 30 THEN FSaldeud
                                WHEN DATE_DIFF(CC.FechaFoto, AA.FechaFoto, MONTH) =  8  AND MoraAc >= 30 THEN FSaldeud
                                WHEN DATE_DIFF(CC.FechaFoto, AA.FechaFoto, MONTH) =  9  AND MoraAc >= 30 THEN FSaldeud
                                WHEN DATE_DIFF(CC.FechaFoto, AA.FechaFoto, MONTH) =  10 AND MoraAc >= 30 THEN FSaldeud
                                ELSE 0 END AS DeudaMorosa30,
     ...
     RESUMEN AS (SELECT *
                 ...
                 PIVOT(COUNT(IdCliente)             AS N,
                       SUM(FCupoAnterior)           AS FCupoAnterior,
                       SUM(FCupoActual)             AS FCupoActual,
                       SUM(MontoAumento)            AS MontoAumento,
                       SUM(Porc_Aumento)            AS Porc_Aumento,
                       SUM(DeudaL1)                 AS DeudaL1,
                       SUM(Utilizacion)             AS Utilizacion,
                       SUM(DeudaMorosa30)           AS DeudaMorosa30,
                       SUM(DeudaTotalL1)            AS DeudaTotalL1
                       
                       FOR Mes IN("Mmenos1","M0","M1","M2","M3","M4","M5","M6","M7","M8","M9","M10")
                       )
